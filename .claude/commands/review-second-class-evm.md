# Second-Class EVM Chain Review

Review a PR that integrates a new second-class EVM chain. Performs exhaustive gap analysis to ensure the chain is wired everywhere it needs to be.

## Usage
Invoke with: `/review-second-class-evm <PR number or URL>`

## Checklist - All Integration Points

### Phase 1: Core Infrastructure (must be in the PR)

For each item, grep the codebase and verify the new chain is present:
...
14. **Asset Generation** - `scripts/generateAssetData/<chain>/index.ts`
    ...
    - **Shared coingecko script** - `scripts/generateAssetData/coingecko.ts` must import and use the new chain's `chainId` constant (used by the token discovery pipeline across all chains)
...
16c. **Market Service Test** - `src/lib/market-service/coingecko/coingecko.test.ts`
    ...
    - Without this, the test will fail on CI

### Phase 2: State & UI Integration (must be in the PR)
...
### Phase 3: Swapper & Data Provider Integration (research required)
...
40. **Related Asset Index** - `public/generated/relatedAssetIndex.json` + `scripts/generateAssetData/generateRelatedAssetIndex/generateRelatedAssetIndex.ts`
    - If the native asset is ETH: verify it's in `manualRelatedAssetIndex[ethAssetId]` array (so ETH on the new chain shows as a related asset of mainnet ETH)
    - Verify `relatedAssetIndex.json` has been regenerated and contains entries for the new chain's tokens
    - Check `generatedAssetData.json` - the chain's ERC20 tokens should have `relatedAssetKey` values (not `None`) linking them to their mainnet counterparts
    - Without this, tokens won't appear in the trade modal "Popular Assets" section and ETH on the chain won't show as a related ETH asset
...
41. **Trade modal "Popular Assets" verification** - after enabling the feature flag, open the trade modal "To" asset selector and filter by the new chain
    ...
    - If only the native asset and a handful of top-100 global market cap tokens show, the related asset index likely wasn't regenerated
    - The `useGetPopularAssetsQuery` hook fetches top 100 global assets then resolves `relatedAssetIds` - tokens only appear if they're linked via `relatedAssetIndex.json`

### Phase 5: Runtime Testing (if running locally)
...
- Verify trade modal "Popular Assets" section shows popular tokens for the chain without searching (USDC, USDT, LINK etc.)
...
3. For Phase 3, web search each service to confirm chain support before flagging
4. Post review with findings using `preferably-blocking:` and `suggestion:` severity prefixes
5. Include runtime testing results if tested locally
